...
export function SettingsPage() {
  ...
  const handleReset = () => {
    const confirmed = window.confirm(copy.settings.restartConfirm);
    if (!confirmed) {
      return;
    }
  ...
  return (
    <MobileShell>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-5">
        ...
        <h1 className="mt-5 text-2xl font-black text-broker-ink">{copy.settings.title}</h1>
        <p className="mt-1 text-sm text-broker-muted">{copy.settings.dataVersion(config.dataVersion)}</p>
